for($i = 0; $i < count($k); $i++ )
    {
       ...
       if(is_numeric($k[$i]))
       {
           $query = query("SELECT * FROM places WHERE MATCH (postal_code) AGAINST (?)", $k[$i]);
       }
    }
        ...
        for($i = 0; $i < count($k); $i++ )
        {
            ...
            $query = query("SELECT * FROM places WHERE MATCH (place_name, admin_name1) AGAINST (?)", $k[$i]);
        }
...
$query = query("SELECT * FROM places WHERE MATCH (place_name, admin_code1) AGAINST ('$city', '$state')");
